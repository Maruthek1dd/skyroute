...
def cliente_existe(id_cliente):
    ...
    resultado = ejecutar_select(query, (id_cliente,))
...
def listar_clientes():
    ...
    try:
        clientes = ejecutar_select(query)
        if not clientes:
        ...
        print("--------------------------------------")
        for c in clientes:
            print(f"{c['id_cliente']} | {c['razon_social']} | {c['cuit']} | {c['email']}")
    except Exception as e:
...
def modificar_cliente():
    ...
    query = "UPDATE clientes SET email = %s WHERE id_cliente = %s"
    try:
